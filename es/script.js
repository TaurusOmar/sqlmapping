...
function copyCommand() {
    ...
    if (targetUrl && !isValidURL(targetUrl)) {
        hasError = true;
        errorMessages.push('Por favor, ingresa una URL válida en el campo "URL Objetivo (-u)".');
    ...
    if (proxy && !isValidURL(proxy)) {
        hasError = true;
        errorMessages.push('Por favor, ingresa una URL válida en el campo "Proxy (--proxy)".');
    ...
    navigator.clipboard.writeText(plainCommand).then(() => {
        ...
        showNotification('Error al copiar el comando: ' + err, true);
    });
}
